...
export const listNovels = {
    definition: {
        name: "list_novels",
        description: "列出当前账号下的所有小说（book_id、书名、字数、总阅读数、连载状态）。多本小说时用它来选择操作目标。",
...
export const switchNovel = {
    definition: {
        name: "switch_novel",
        description: "切换当前要操作的小说（传 book_id，可先用 list_novels 查）。之后的发布、数据查询默认作用于这本，直到再次切换。",
        inputSchema: {
            type: "object",
    ...
    async run(args) {
        ...
        if (!novel)
            throw new Error(`账号下找不到 book_id=${bookId} 的小说`);
        setCurrentBookId(bookId);
        return { switched_to: bookId, book_name: novel.book_name };
    },
};
export const getCurrentNovel = {
    definition: {
